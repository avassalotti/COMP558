...
distance function---this is analogous to a $p$-norm. For the level set to
evolve to a local minimum of this energy functional, it has to move according
to a force which employs not only $d(\mathbf{x})$ but also the normal to
$\Gamma$ and the mean curvature $\kappa$ of $\Gamma$. In fact, in can be shown
that the minimizer, or steady state, of \eqref{eq:functional} satisfies:
\[
